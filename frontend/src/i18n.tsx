...
const translations = {
  en: {
    ...
    'llm.apiKeyHintOpenAI': 'Get an OpenAI API key',
    'llm.apiKeyHintClaude': 'Get an Anthropic API key',
    'llm.apiKeyHintPrefix': "Don't have one?",
    'llm.modelListLoading': 'Fetching latest models from provider…',
    'llm.modelListLive': 'Model list fetched live from your provider.',
    'llm.modelListFallback': 'Showing built-in defaults. Paste an API key to load the latest models.',
    'llm.modelListError': 'Could not fetch models live — using built-in defaults.',
    ...
    'explain.hide': 'Hide',
    'explain.hint': 'Click a term name to see the formula, meaning, and value interpretation.',
    'explain.calculation': 'How It Is Calculated',
    'explain.meaning': 'What It Means',
    'explain.interpretation': 'How to Read High vs Low',
    'explain.current': 'Current Reading',
  ...
  zh: {
    ...
    'llm.apiKeyHintOpenAI': '获取 OpenAI API Key',
    'llm.apiKeyHintClaude': '获取 Anthropic API Key',
    'llm.apiKeyHintPrefix': '还没有？',
    'llm.modelListLoading': '正在从服务商获取最新模型列表…',
    'llm.modelListLive': '模型列表已实时同步自你的服务商账号。',
    'llm.modelListFallback': '当前显示内置默认列表，填入 API Key 后会自动加载最新模型。',
    'llm.modelListError': '获取最新模型失败，已回退到内置列表。',
    ...
    'explain.show': '解释',
    'explain.hide': '收起',
    'explain.hint': '点击术语名称可查看公式、含义和数值高低的解读。',
    'explain.calculation': '怎么算出来的',
    'explain.meaning': '代表什么',
    'explain.interpretation': '数值高低怎么看',
    'explain.current': '当前读数',
    'news.title': '近期新闻',
    'news.empty': '暂无新闻',
    'news.justNow': '刚刚',
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  const translateSignalName = (name: string) => {
    const labels: Record<string, { en: string; zh: string }> = {
      ...
      support_resistance: { en: 'Support / Resistance', zh: '支撑 / 阻力' },
      ...
      relative_strength_spy: { en: 'Relative Strength vs SPY', zh: '相对 SPY 强弱' },
      ...
      earnings_proximity: { en: 'Earnings Proximity', zh: '财报临近度' },
      volatility_regime: { en: 'Volatility Regime', zh: '波动率状态' },
      sector_etf_trend: { en: 'Sector ETF Trend', zh: '行业 ETF 趋势' },
  ...
  const translateSignalRationale = (name: string, rationale: string) => {
    ...
    const genericPatterns: Array<[RegExp, (...args: string[]) => string]> = [
      ...
      [/^RSI ([\d.]+) — oversold(?: \(contrarian: mean-reversion buy\))?$/, (value) => `RSI ${value}，超卖`],
      [/^RSI ([\d.]+) — approaching oversold(?: \(contrarian\))?$/, (value) => `RSI ${value}，接近超卖`],
      [/^RSI ([\d.]+) — neutral$/, (value) => `RSI ${value}，中性`],
      [/^RSI ([\d.]+) — approaching overbought(?: \(contrarian\))?$/, (value) => `RSI ${value}，接近超买`],
      [/^RSI ([\d.]+) — overbought(?: \(contrarian: mean-reversion sell\))?$/, (value) => `RSI ${value}，超买`],
      ...
      [/^OBV slope ([\d.+-]+), price up — confirmed$/, (value) => `OBV 斜率 ${value}，价格上涨，量价确认`],
      [/^OBV slope ([\d.+-]+), price down — confirmed$/, (value) => `OBV 斜率 ${value}，价格下跌，量价确认`],
      [/^OBV slope ([\d.+-]+), price down — (confirmed|weak) positive divergence \((\d)\/3\)$/, (value, strength, count) => `OBV 斜率 ${value}，价格下跌，${strength === 'confirmed' ? '确认' : '轻微'}正背离（${count}/3）`],
      [/^OBV slope ([\d.+-]+), price up — (confirmed|weak) negative divergence \((\d)\/3\)$/, (value, strength, count) => `OBV 斜率 ${value}，价格上涨，${strength === 'confirmed' ? '确认' : '轻微'}负背离（${count}/3）`],
      [/^OBV slope ([\d.+-]+), neutral$/, (value) => `OBV 斜率 ${value}，中性`],
      ...
      [/^Relative strength vs SPY \((.+)\)$/, (value) => `相对 SPY 强弱（${value}）`],
      [/^Earnings growth: ([+\-]?\d+(?:\.\d+)?)%$/, (value) => `利润增长：${value}%`],
      [/^Revenue growth: ([+\-]?\d+(?:\.\d+)?)%$/, (value) => `营收增长：${value}%`],
      [/^Earnings in (\d+)d — high uncertainty$/, (days) => `${days} 天后发布财报，不确定性高`],
      [/^Earnings in (\d+)d — vol expansion risk$/, (days) => `${days} 天后发布财报，存在波动放大风险`],
      [/^Earnings in (\d+)d — low near-term event risk$/, (days) => `${days} 天后发布财报，短期事件风险较低`],
      ...
      [/^Sector ETF ([+\-]?\d+(?:\.\d+)?)% from MA50, MA50 rising$/, (value) => `行业 ETF 相对 MA50 偏离 ${value}%，且 MA50 上行`],
      [/^Sector ETF ([+\-]?\d+(?:\.\d+)?)% from MA50, MA50 falling$/, (value) => `行业 ETF 相对 MA50 偏离 ${value}%，且 MA50 下行`],
      [/^Trailing P\/E = ([\d.]+)(?: \(sector: (.+)\))?$/, (value, sector) => sector ? `静态市盈率 = ${value}（行业：${sector}）` : `静态市盈率 = ${value}`],
      [/^P\/B = ([\d.]+)(?: \(sector: (.+)\))?$/, (value, sector) => sector ? `市净率 = ${value}（行业：${sector}）` : `市净率 = ${value}`],
      [/^PEG = ([\d.]+)$/, (value) => `PEG = ${value}`],
      [/^EV\/EBITDA = ([\d.]+)(?: \(sector: (.+)\))?$/, (value, sector) => sector ? `EV/EBITDA = ${value}（行业：${sector}）` : `EV/EBITDA = ${value}`],
      [/^Revenue growth \(latest quarter YoY\): ([+\-]?\d+(?:\.\d+)?)%$/, (value) => `最新季度营收同比增长：${value}%`],
      [/^Earnings growth \(latest quarter YoY\): ([+\-]?\d+(?:\.\d+)?)%$/, (value) => `最新季度利润同比增长：${value}%`],
      [/^ROE = ([+\-]?\d+(?:\.\d+)?)%$/, (value) => `ROE = ${value}%`],
      [/^ROIC = ([+\-]?\d+(?:\.\d+)?)% \((improving|declining|single period)\)$/, (value, trend) => `ROIC = ${value}%（${trend === 'improving' ? '改善中' : trend === 'declining' ? '走弱中' : '单期数据'}）`],
      [/^(FCF margin|Profit margin \(FCF unavailable\)): ([+\-]?\d+(?:\.\d+)?)%$/, (label, value) => `${label === 'FCF margin' ? '自由现金流利润率' : '利润率（缺少 FCF，使用回退口径）'}：${value}%`],
      ...
      [/^Structural events \((\d+)\): (.+)$/, (count, value) => `结构性事件（${count}）：${value}`],
    ]
